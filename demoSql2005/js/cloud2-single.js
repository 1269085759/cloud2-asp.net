...
function HttpUploaderMgr()
{
	...
	this.Config = {
        ...
        , "Cloud"           : null//云存储
		//文件夹操作相关
		, "UrlFdCreate"		: "http://localhost:4960/demoSql2005/db/fd_create.aspx"
		, "UrlFdComplete"	: "http://localhost:4960/demoSql2005/db/fd_complete.aspx"
		, "UrlFdDel"	    : "http://localhost:4960/demoSql2005/db/fd_del.aspx"
		, "UrlFdFile"	    : "http://localhost:4960/demoSql2005/db/fd_file.aspx"
		//文件操作相关
		, "UrlCreate"		: "http://localhost:4960/demoSql2005/db/f_create.aspx"
		, "UrlProcess"		: "http://localhost:4960/demoSql2005/db/f_process.aspx"
		, "UrlComplete"		: "http://localhost:4960/demoSql2005/db/f_complete.aspx"
		, "UrlList"			: "http://localhost:4960/demoSql2005/db/f_list.aspx"
		, "UrlDel"			: "http://localhost:4960/demoSql2005/db/f_del.aspx"
	    //x86
        , ie: {
              drop: { clsid: "E64B88F9-B1F8-4945-96DC-329215D07096", name: "Xproer.CloudDroper2" }
            , part: { clsid: "FD71C5F4-BE1D-4339-8F15-22D41ECCCF9B", name: "Xproer.CloudPartition2" }
            , path: "http://www.ncmem.com/download/cloud2/cloud2.cab"
        }
	    //x64
        , ie64: {
              drop: { clsid: "E596A35A-2890-4A01-9230-BA8AFA5B2C62", name: "Xproer.CloudDroper2x64" }
            , part: { clsid: "17207E42-2B9C-4916-83B5-1031EC0E6A09", name: "Xproer.CloudPartition2x64" }
            , path: "http://www.ncmem.com/download/cloud2/cloud64.cab"
        }
        , firefox: { name: "", type: "application/npCloudUp2", path: "http://www.ncmem.com/download/cloud2/cloud2.xpi" }
        , chrome: { name: "npCloudUp2", type: "application/npCloudUp2", path: "http://www.ncmem.com/download/cloud2/cloud2.crx" }
        , chrome45: { name: "com.xproer.cloud2", path: "http://www.ncmem.com/download/cloud2/cloud2.nat.crx" }
        , exe: { path: "http://www.ncmem.com/download/cloud2/cloud2.exe" }
	...
	this.load_complete = function () { if (this.btnSetup) this.btnSetup.hide(); };
	...
	this.recvMessage = function (str)
	{
	    ...
	    else if (json.name == "post_part") { _this.post_part(json); }
	...
	this.browser = {
        ...
        , initNat: function ()
        {
            ...
            this.exitEvent();
            document.addEventListener(this.entCbkID, function (evt)
            {
...
function FileUploader(fileLoc, mgr)
{
    ...
    this.post_part = function (json)
    {
        var param = jQuery.extend({}, this.fileSvr);
        ...
        $.ajax({
            ...
            {
                //alert("更新文件进度失败！" + req.responseText);
            }
            , complete: function (req, sta) { req = null; }
        });
    };
    ...
    this.md5_complete = function (json)
    {
        ...
        var cloud_str = JSON.stringify(this.Config.Cloud);
        var param = jQuery.extend({}, this.fields, { cloud: cloud_str, md5: json.md5, lenLoc: loc_len, sizeLoc: loc_size, pathLoc: loc_path, time: new Date().getTime() });
